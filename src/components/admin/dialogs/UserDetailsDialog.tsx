...
export function UserDetailsDialog({ user, open, onOpenChange }: UserDetailsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-slate-800 border-emerald-800/30 text-white max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="bg-emerald-600/20 text-emerald-400 px-3 py-2 rounded-lg">
            ...
            <div>
              <label className="text-slate-400 text-sm">Email Id</label>
              <p className="text-white font-medium">{user.email}</p>
            </div>
            <div>
              <label className="text-slate-400 text-sm">Referral Level</label>
              <p className="text-emerald-400 font-medium">N/A</p>
            </div>
            <div>
              <label className="text-slate-400 text-sm">Lock Period</label>
          ...
          <div>
            ...
            <div className="bg-slate-900/50 rounded-lg p-4 space-y-3">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-slate-400 text-sm">ASVO Balance</label>
                  <p className="text-emerald-400 font-medium">{user.asvoBalance || '0'} ASVO</p>
                </div>
                <div>
                  <label className="text-slate-400 text-sm">ASVO Reward Balance</label>
                  <p className="text-emerald-400 font-medium">{user.asvoRewardBalance || '0'} ASVO</p>
                </div>
                <div>
                  <label className="text-slate-400 text-sm">ASVO Referral Balance</label>
                  <p className="text-emerald-400 font-medium">{user.asvoReferralBalance || '0'} ASVO</p>
                </div>
                <div>
                  <label className="text-slate-400 text-sm">USDT (BEP20)</label>
                  <p className="text-white font-medium">${user.usdtBep20Balance || '0'}</p>
                </div>
                <div>
                  <label className="text-slate-400 text-sm">USDT (TRC20)</label>
                  <p className="text-white font-medium">${user.usdtTrc20Balance || '0'}</p>
                </div>
                <div>
                  <label className="text-slate-400 text-sm">ASVO (Staked)</label>
